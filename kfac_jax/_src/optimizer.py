...
from jax import lax
from kfac_jax._src import curvature_estimator, utils
...
class Optimizer(utils.WithStagedMethods):
    ...
    class State(Generic[Params], utils.State):  # type: ignore
    ...
    def __init__(
        ...
        custom_estimator_ctor: Callable[..., BlockDiagonalCurvature] | None = None,
        ...
        if curvature_update_period > inverse_update_period:
            raise ValueError(
                ...
                " be redundantly performed. Set inverse_update_period larger instead.".format(
                    curvature_update_period, inverse_update_period
                )
            )
        ...
        if (
            norm_to_scale_identity_weight_per_block is not None
            and norm_to_scale_identity_weight_per_block != "none"
        ):

            assert (
                not use_adaptive_learning_rate
                and not use_adaptive_momentum
    ...
    def _verify_args_and_get_step_counter(
        ...
        if self._use_adaptive_learning_rate and learning_rate is not None:
            raise ValueError(
                "When use_adaptive_learning_rate is set to True you "
            ...
            raise ValueError(
                "When `use_adaptive_learning_rate` is set to False and "
            ...
            raise ValueError(
                "When you have passed a `learning_rate_schedule` you "
                "should not pass a value to the step function."
            )

        if self._use_adaptive_momentum and momentum is not None:
            raise ValueError(
                "When `use_adaptive_momentum` is set to True you "
                "should not pass a value to the step function."
            )
            ...
            raise ValueError(
                "When `use_adaptive_momentum` is set to False and "
            ...
            raise ValueError(
                "When you have passed a `momentum_schedule` you should "
                "not pass a value to the step function."
            )

        if self._use_adaptive_damping and damping is not None:
            raise ValueError(
                "When `use_adaptive_damping` is set to True you "
                "should not pass a value to the step function."
            )
            ...
            raise ValueError(
                "When `use_adaptive_damping` is set to False and "
            ...
            raise ValueError(
                "When you have passed a `damping_schedule` you should "
                "not pass a value to the step function."
            )
    ...
    def _update_estimator_curvature(
        ...
        return jax.lax.cond(
            sync,
            functools.partial(self._estimator.sync, pmap_axis_name=self.pmap_axis_name),
    ...
    def _maybe_update_inverse_cache(self, state: State, precon_damping: Array) -> State:
    ...
    @utils.staged
    def _compute_loss_from_registrations(self, func_args: FuncArgsVariants) -> Array:
        ...
        if self._l2_reg > 0.0:
            ...
            l2_reg_val = (
                self._l2_reg / 2 * utils.squared_norm(func_args[self._params_index])
            )
    ...
    def _init(
        ...
        return Optimizer.State(
            ...
            damping=jnp.array(
                (self._initial_damping if self._initial_damping is not None else -1e10),
                dtype=float,
            ),
    ...
    @functools.partial(
        utils.staged, donate_argnums=[1, 3, 5]
    ...
    def _burnin(
        ...
        func_args, rng = self._setup_func_args_and_rng(params, rng, batch, func_state)
    ...
    @functools.partial(utils.staged, donate_argnums=(0, 1, 4), static_argnums=(8, 9))
    @utils.auto_scope_method
    def _step(
        self,
        params: Params,
        ...
        ######################################################################
        ######################################################################
        if self.mcmc_step_fn is not None:
        ...
        ######################################################################
        ######################################################################
        # Calculate new loss if needed
        if self._use_step_rejection or self.adapt_Delta or should_update_damping:
            assert self.loss_func is not None
            assert self._value_func_has_aux is True
            assert self._value_func_has_state is False
            assert self._value_func_has_rng is False

            # Run MCMC to update batch before computing new loss if self.mcmc_step_fn is provided
            if self.mcmc_step_fn is not None:
                new_batch, _ = self.mcmc_step_fn(
                    new_params,
                    batch,
                    mcmc_key,
                    state.move_widths,
        ...
        if self.adapt_Delta and self.use_trust_region:
            is_good = jnp.asarray(new_loss < (loss - self.k_grow * aux.se), dtype=bool)
            is_bad = jnp.asarray(new_loss > (loss + self.k_shrink * aux.se), dtype=bool)
            state.Delta = jax.lax.select(
                jnp.logical_and(is_finite, is_good),
                jnp.minimum(state.Delta * self.grow, self.Delta_max),
                jax.lax.select(
                    jnp.logical_or((~is_finite), is_bad),
                    jnp.maximum(state.Delta * self.shrink, self.Delta_min),
        ...
        if should_update_damping:
            ...
            rho = self._invalid_metric_value
        ...
        if self._include_registered_loss_in_stats:
            ...
            stats["loss_registered"] = utils.pmean_if_pmap(
                stats["loss_registered"], self.pmap_axis_name
            )
            stats["loss_registered_reldiff"] = (stats["loss_registered"] - loss) / loss
    ...
    def step(
        ...
        if (data_iterator is None) == (batch is None):
            raise ValueError(
                "Exactly one of the arguments ``data_iterator`` and "
        ...
        if step_counter_int == 0:
            ...
            if self._num_burnin_steps > 0:
                ...
                if data_iterator is None:
                    raise ValueError(
                        "If num_burnin_steps > 0, data_iterator must be " "provided."
                    )
        ...
        if (
            step_counter_int == 0
            and self._use_adaptive_damping
    ...
    def _calibrate_initial_damping(
        ...
        while True:
            ...
            # these arguments.
            ret = self._step(
                self.copy_obj(params),
                self.copy_obj(state),
                rng,
                batch,
                self.copy_obj(func_state),
                learning_rate,
                momentum,
                None,
                False,
    ...
    def _compute_exact_quad_model_filtered(
        ...
        **kwargs,
        ...
        if isinstance(fixed_coefficients[1], float) and fixed_coefficients[1] == 0.0:
    ...
    def _maybe_mask_out_unregularized_parameters(
        ...
        if log_paths:
            logging.info(
                "Unregularized parameters masking info (for curvature "
                "calculations and L2 regularization)"
            )

        def maybe_mask_out_single_param(path: tuple[Any, ...], param: Array) -> Array:
            ...
            should_mask = any(
                p in str_path for p in self._regularized_parameters_path_exclusions
            )
        ...
        return jax.tree.map_with_path(maybe_mask_out_single_param, params)
    ...
    def _compute_exact_quad_model(
        ...
        if self._estimator.default_mat_type == "fisher":
            c_factor_v = tuple(
                self._implicit.multiply_fisher_factor_transpose(func_args, vi)
                for vi in vectors
            )
        elif self._estimator.default_mat_type == "ggn":
            c_factor_v = tuple(
                self._implicit.multiply_ggn_factor_transpose(func_args, vi)
                for vi in vectors
            )
        else:
            raise ValueError(
                f"Unrecognized estimator.mat_type="
                f"{self._estimator.default_mat_type}."
            )

        masked_vectors = tuple(
            self._maybe_mask_out_unregularized_parameters(vi) for vi in vectors
        )
    ...
    def _compute_approx_quad_model(
        ...
        return (
            utils.symmetric_matrix_inner_products(c_vectors, vectors),
    ...
    def _solve_quad_model(
        ...
        quad_model_change = self._evaluate_quadratic_model(A, A_damped, b, jnp.array(w))
